...
final class AESTests: XCTestCase {
    ...
    func testDecrypt() {
        guard let aes = AES(key: aesKey, blockMode: .ECB) else {
            return XCTAssert(false, "failed")
        }
        
        let input:[UInt8] = [0x69, 0xc4, 0xe0, 0xd8,
            0x6a, 0x7b, 0x4, 0x30,
            0xd8, 0xcd, 0xb7, 0x80,
            0x70, 0xb4, 0xc5, 0x5a];

        let expected:[UInt8] = [0x00, 0x11, 0x22, 0x33,
            0x44, 0x55, 0x66, 0x77,
            0x88, 0x99, 0xaa, 0xbb,
            0xcc, 0xdd, 0xee, 0xff];
        
        let decrypted = try! aes.decrypt(input, padding: nil)
    ...
    func testComputeTables() {
        let aes = AES(key: aesKey, blockMode: .CBC)
        aes?.computeKeySchedule()
    }
