    ...
    func toUInt32Array(bigEndian bigEndian: Bool = true) -> Array<UInt32> {
        ...
        for idx in self.startIndex.stride(to: self.endIndex, by: sizeof(UInt32)) {
            ...
            if bigEndian {
                ...
                val |= UInt32(self[idx.advancedBy(3)] as! UInt8) << 0
                val |= UInt32(self[idx.advancedBy(2)] as! UInt8) << 8
                val |= UInt32(self[idx.advancedBy(1)] as! UInt8) << 16
                val |= UInt32(self[idx.advancedBy(0)] as! UInt8) << 24
            }
            result.append(val)
        }
        return result
